              ...
              <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 scrollbar-thin scrollbar-thumb-zinc-200">
                {stageTickets.length === 0 ? (
                  ...
                  stageTickets.map(ticket => (
                    ...
                    >
                      {/* Top: Ticket Number & Model */}
                      <div className="flex items-center justify-between">
                        <span className="font-mono font-bold text-[#ff4b16] text-xs">
                          {ticket.ticketNumber}
                        </span>
                      ...
                      {/* Passcode & Tech Assignee */}
                      <div className="pt-1.5 border-t border-zinc-100 flex items-center justify-between text-[10px]">
                        <div className="flex items-center space-x-1 text-zinc-500">
                          <User className="w-3 h-3 text-zinc-400" />
                          <span>KTV: <strong className="text-zinc-700">{ticket.technician || 'KTV'}</strong></span>
                        </div>

                        {ticket.passcode && (
                          <div className="flex items-center space-x-0.5 font-mono font-bold text-zinc-800 bg-zinc-100 px-1 py-0.2 rounded">
                            <Lock className="w-2.5 h-2.5 text-zinc-400" />
                            <span>{ticket.passcode}</span>
                          </div>
                        )}
                      </div>
                    </div>
