...
import {
  ...
  ClipboardCheck, ImagePlus, Loader2, PackageCheck, ShieldCheck,
...
const initialForm = () => ({
  ...
  deviceAppearance: '', accessoriesIncluded: '', passcode: '', notes: '', priority: 'NORMAL' as Priority
});
...
export const RepairIntakeModal: React.FC<RepairIntakeModalProps> = ({
  ...
  useEffect(() => {
    ...
    setStep(1); setPhotos([]); setSelectedTaskTypes([]); setError('');
  ...
  const validateStep = (target: IntakeStep): boolean => {
    ...
    if (target === 2) {
      ...
      if (photos.length < 1) { setError('Cần ít nhất 1 ảnh tình trạng máy khi tiếp nhận.'); return false; }
    }
    if (target === 3) {
  ...
  const submit = async () => {
    ...
    try {
      const result = await requestCreateWorkOrder({
        ...
        intakeDetails: { issueType: form.issueType, faultDescription: form.faultDescription.trim(), deviceAppearance: form.deviceAppearance.trim() || 'Chưa ghi nhận', accessoriesIncluded: form.accessoriesIncluded.trim() || 'Máy trần', expectedReturnDate: form.expectedReturnDate || undefined },
      ...
      const intakePhotoUrls = await uploadTechnicalEvidence(result.workOrderId, 'INTAKE', photos);
      await requestAttachIntakeEvidence(result.workOrderId, intakePhotoUrls);
  ...
  const addPhotos = (files: FileList | null) => {
    ...
    if (invalid) { setError('Chỉ nhận ảnh dưới 10MB mỗi tệp.'); return; }
  ...
  return <div className="fixed inset-0 z-[120] flex items-end bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:justify-center sm:p-4">
    <div className="flex h-[94dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-3xl bg-zinc-50 shadow-2xl sm:h-auto sm:max-h-[92vh] sm:rounded-3xl">
      ...
      <footer className="flex items-center justify-between gap-3 border-t border-zinc-200 bg-white p-3 sm:p-4"><button type="button" onClick={() => step === 1 ? onClose() : setStep(current => (current - 1) as IntakeStep)} className="inline-flex items-center gap-1 rounded-xl px-3 py-2 text-xs font-bold text-zinc-600 hover:bg-zinc-100">{step === 1 ? 'Hủy' : <><ChevronLeft className="h-4 w-4" />Quay lại</>}</button>{step < 4 ? <button type="button" disabled={loadingSettings && step === 3} onClick={() => { if (validateStep(step)) setStep(current => (current + 1) as IntakeStep); }} className="inline-flex items-center gap-1 rounded-xl bg-zinc-950 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">Tiếp tục<ChevronRight className="h-4 w-4" /></button> : <button type="button" disabled={saving} onClick={() => void submit()} className="inline-flex items-center gap-2 rounded-xl bg-orange-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}{saving ? 'Đang tạo & tải ảnh...' : 'Tạo phiếu tiếp nhận'}</button>}</footer>
    </div>
  </div>;
};
