...
export const StockItemPurchaseEntryForm: React.FC<StockItemPurchaseEntryFormProps> = ({
  ...
  return (
    <div data-ph-fullscreen-form className="fixed inset-0 z-[110] flex h-[100dvh] w-screen items-end bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:justify-center sm:p-5">
      <div className="flex h-[100dvh] w-full flex-col overflow-hidden bg-zinc-50 shadow-2xl sm:h-auto sm:max-h-[94dvh] sm:max-w-6xl sm:rounded-3xl">
        ...
        <main className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5">
          ...
          {step === 'CONTEXT' && <section className="mx-auto max-w-4xl space-y-4">
          ...
          {step === 'ITEMS' && <section className="mx-auto max-w-5xl space-y-4">
          ...
          {step === 'PAYMENT' && <section className="mx-auto max-w-4xl space-y-4"><div><h3 className="text-lg font-black text-zinc-900">Thanh toán &amp; công nợ</h3><p className="mt-1 text-xs text-zinc-500">Chọn một nguồn tiền để chi ngay hoặc để toàn bộ số tiền thành công nợ nhà cung cấp.</p></div><section className="rounded-2xl border border-zinc-200 bg-white p-4"><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setPayment('DEBT')} className={`rounded-xl px-3 py-2.5 text-xs font-bold ${payment === 'DEBT' ? 'bg-zinc-950 text-white' : 'border border-zinc-200 text-zinc-600'}`}><Building2 className="mr-1 inline h-3.5 w-3.5" />Ghi nợ NCC</button><button type="button" onClick={() => setPayment('CASH')} className={`rounded-xl px-3 py-2.5 text-xs font-bold ${payment === 'CASH' ? 'bg-orange-600 text-white' : 'border border-zinc-200 text-zinc-600'}`}><Coins className="mr-1 inline h-3.5 w-3.5" />Tiền mặt</button><button type="button" onClick={() => setPayment('BANK')} className={`rounded-xl px-3 py-2.5 text-xs font-bold ${payment === 'BANK' ? 'bg-sky-600 text-white' : 'border border-zinc-200 text-zinc-600'}`}><CreditCard className="mr-1 inline h-3.5 w-3.5" />Chuyển khoản</button></div>{payment !== 'DEBT' && <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2"><label className="text-xs font-black text-zinc-600">Quỹ / tài khoản chi<select value={fundId} onChange={event => setFundId(event.target.value)} className="mt-1 h-11 w-full rounded-xl border border-zinc-200 bg-white px-3 text-sm font-bold"><option value="">Chọn quỹ/tài khoản</option>{matchingFunds.map(fund => <option key={fund.id} value={fund.id}>{fund.name}</option>)}</select></label><label className="text-xs font-black text-zinc-600">Số tiền thanh toán<input type="number" value={paidAmount} min="0" max={total} onChange={event => setPaidAmount(Number(event.target.value))} className="mt-1 h-11 w-full rounded-xl border border-zinc-200 px-3 text-right text-sm font-bold" /></label></div>}<div className="mt-4 grid grid-cols-3 gap-2 border-t border-zinc-100 pt-4 text-center"><div><p className="text-[10px] font-black uppercase text-zinc-400">Tổng phiếu</p><p className="mt-1 text-sm font-black text-zinc-900">{money(total)} ₫</p></div><div><p className="text-[10px] font-black uppercase text-zinc-400">Chi ngay</p><p className="mt-1 text-sm font-black text-emerald-700">{money(payment === 'DEBT' ? 0 : paidAmount)} ₫</p></div><div><p className="text-[10px] font-black uppercase text-zinc-400">Còn nợ</p><p className="mt-1 text-sm font-black text-rose-600">{money(debt)} ₫</p></div></div></section></section>}

          {step === 'REVIEW' && <section className="mx-auto max-w-4xl space-y-4"><div><h3 className="text-lg font-black text-zinc-900">Kiểm tra trước khi nhập kho</h3><p className="mt-1 text-xs text-zinc-500">Đây là lần kiểm tra cuối. Khi xác nhận, hệ thống sẽ ghi toàn bộ chứng từ trong một giao dịch.</p></div><section className="rounded-2xl border border-zinc-200 bg-white p-4"><div className="grid gap-3 text-sm sm:grid-cols-2"><div><p className="text-[10px] font-black uppercase text-zinc-400">Nhà cung cấp</p><p className="mt-1 font-black text-zinc-900">{selectedSupplier?.name || '—'}</p></div><div><p className="text-[10px] font-black uppercase text-zinc-400">Kho nhận</p><p className="mt-1 font-black text-zinc-900">{selectedWarehouse?.name || '—'}</p></div><div><p className="text-[10px] font-black uppercase text-zinc-400">Số dòng / số lượng</p><p className="mt-1 font-black text-zinc-900">{lines.length} SKU · {resolvedLines.reduce((sum, entry) => sum + Number(entry.line.quantity), 0)} đơn vị</p></div><div><p className="text-[10px] font-black uppercase text-zinc-400">Thanh toán</p><p className="mt-1 font-black text-zinc-900">{payment === 'DEBT' ? 'Ghi nhận công nợ NCC' : `${payment === 'CASH' ? 'Tiền mặt' : 'Chuyển khoản'} · ${money(paidAmount)} ₫`}</p></div></div><div className="mt-4 divide-y rounded-xl border border-zinc-100">{resolvedLines.map(({ line, item }) => <div key={line.key} className="flex items-center justify-between gap-3 px-3 py-2.5 text-xs"><div className="min-w-0"><p className="truncate font-black text-zinc-900">{item.name}</p><p className="font-mono text-[10px] text-zinc-500">{item.sku} · SL {line.quantity} · Bán {money(line.retailPrice)} ₫</p></div><p className="shrink-0 font-black text-zinc-900">{money(line.quantity * line.unitCost)} ₫</p></div>)}</div><div className="mt-4 flex items-center justify-between border-t border-zinc-100 pt-4"><span className="text-sm font-bold text-zinc-500">Tổng giá trị phiếu</span><span className="text-xl font-black text-orange-700">{money(total)} ₫</span></div></section></section>}
        </main>

        <footer className="flex items-center justify-between gap-3 border-t border-zinc-200 bg-white p-4"><button type="button" onClick={() => currentStepIndex === 0 ? onClose() : setStep(RECEIPT_STEPS[currentStepIndex - 1].id)} className="inline-flex h-11 items-center gap-1.5 rounded-xl border border-zinc-200 bg-white px-4 text-sm font-black text-zinc-700"><ChevronLeft className="h-4 w-4" /> {currentStepIndex === 0 ? 'Hủy' : 'Quay lại'}</button>{step !== 'REVIEW' ? <button type="button" onClick={moveToNext} className="ml-auto inline-flex h-11 items-center gap-1.5 rounded-xl bg-orange-600 px-4 text-sm font-black text-white shadow-sm hover:bg-orange-700">Tiếp tục <ChevronRight className="h-4 w-4" /></button> : <button type="button" disabled={submitting} onClick={() => void submit()} className="ml-auto inline-flex h-11 items-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white shadow-sm hover:bg-emerald-700 disabled:opacity-50">{submitting ? 'Đang ghi phiếu…' : <><CheckCircle2 className="h-4 w-4" /> Xác nhận nhập hàng</>}</button>}</footer>
      </div>
    </div>
  );
};
