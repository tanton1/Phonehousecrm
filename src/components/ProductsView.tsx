...
export const ProductsView: React.FC<ProductsViewProps> = ({
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-4 pb-24 sm:space-y-5 sm:pb-8">
      ...
      {dialogOpen && <div className="fixed inset-0 z-50 flex items-end bg-zinc-950/45 sm:items-center sm:justify-center sm:p-5"><section className="flex max-h-[92dvh] w-full max-w-2xl flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl"><header className="flex items-center justify-between border-b px-4 py-4 sm:px-6"><div><h2 className="text-base font-black text-zinc-900">{formData.id ? 'Cập nhật tồn và giá' : 'Thêm tồn từ Danh mục'}</h2><p className="mt-0.5 text-xs text-zinc-500">{formData.id ? 'SKU thuộc Danh mục nên không sửa tại kho.' : 'Chọn mã có sẵn, không tạo SKU mới tại đây.'}</p></div><button type="button" onClick={() => setDialogOpen(false)} className="rounded-xl p-2 text-zinc-500"><X className="h-5 w-5" /></button></header><div className="flex-1 space-y-4 overflow-y-auto p-4 sm:p-6">
        ...
        {formData.id && <section className="rounded-2xl border bg-zinc-50 p-3"><p className="font-mono text-xs font-black text-orange-700">{formData.sku}</p><p className="mt-1 text-sm font-black text-zinc-900">{formData.name}</p><p className="mt-1 text-[11px] text-zinc-500">Muốn đổi tên/SKU: mở Danh mục hàng hóa.</p></section>}
