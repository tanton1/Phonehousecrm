        ...
        <div className="flex flex-wrap items-center gap-2">
          {onOpenAddTaskModal && (
            <button
              ...
              className="px-3 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer"
            >
              + Phân Công Task KTV
            </button>
          )}
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 border border-zinc-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 w-full sm:w-64"
            />
          </div>
              ...
              <div 
                ...
                className={`p-4 rounded-xl border cursor-pointer transition-all ${
                  selectedTech?.userId === stat.userId 
                    ...
                    : 'border-zinc-200 hover:border-rose-300 hover:shadow-sm bg-white'
                }`}
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center space-x-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white shadow-sm ${
                      selectedTech?.userId === stat.userId ? 'bg-rose-600' : 'bg-zinc-800'
                    }`}>
                  ...
                  <div className="text-right">
                    ...
                    <div className={`font-bold ${stat.totalCommission > 0 ? 'text-orange-600' : 'text-zinc-400'}`}>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2 pt-3 border-t border-zinc-100">
                  ...
                  <div className="text-center p-2 rounded-lg bg-orange-50">
                    <div className="text-xs text-orange-600 mb-1">Hoàn Thành</div>
                    <div className="font-semibold text-orange-700">{stat.completedTickets}</div>
                  </div>
                  <div className="text-center p-2 rounded-lg bg-orange-50">
                    <div className="text-xs text-orange-600 mb-1">Đang Xử Lý</div>
                    <div className="font-semibold text-orange-700">{stat.pendingTickets}</div>
                  </div>
                </div>
              </div>
            ))}
              ...
              <div className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-2xl font-bold text-orange-600">{formatCurrency(selectedTech.totalCommission)}</div>
                  </div>
                </div>
                    ...
                    <div key={ticket.id} className="bg-white p-4 rounded-xl border border-zinc-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:shadow-sm transition-all">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          {ticket.taskType === 'INBOUND_QC' ? (
                            <span className="text-[10px] font-semibold text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded">QC Nhập</span>
                          ) : (
                      ...
                      <div className="flex items-center sm:flex-col sm:items-end justify-between sm:justify-center border-t sm:border-t-0 border-zinc-100 pt-3 sm:pt-0 pl-0 sm:pl-4 sm:border-l">
                        <div className="flex flex-col items-start sm:items-end mb-1">
                          ...
                          <span className={`font-bold text-sm ${isCompleted && (ticket.commissionAmount || 0) > 0 ? 'text-orange-600' : 'text-zinc-400'}`}>
                            {isCompleted ? formatCurrency(ticket.commissionAmount || 0) : 'Chưa chốt'}
                          </span>
                        </div>
                        <div className="text-xs">
                          {isCompleted ? (
                            <span className="text-orange-600 flex items-center bg-orange-50 px-2 py-1 rounded font-medium">
