        ...
        <div className="flex-1 overflow-auto bg-zinc-50/50 p-3 sm:p-5">
          {activeTab === 'KANBAN' && (
            <div className="h-full flex flex-col">
              ...
              {pendingHandoffs.length > 0 && <div className="mb-3 rounded-2xl border border-blue-200 bg-blue-50 p-3"><p className="text-xs font-black uppercase tracking-wide text-blue-900">Có {pendingHandoffs.length} máy đang chờ bạn nhận trách nhiệm</p><div className="mt-2 flex flex-wrap gap-2">{pendingHandoffs.map(handoff => <button key={handoff.id} onClick={() => { setSelectedHandoff(handoff); setHandoffScan(''); setHandoffNotes(''); setHandoffFiles([]); }} className="rounded-xl bg-white px-3 py-2 text-left text-xs shadow-sm"><strong className="block text-blue-800"><span data-imei-history={handoff.imei} title={`Xem toàn bộ lịch sử IMEI ${handoff.imei}`} className="underline decoration-dotted underline-offset-2">{handoff.imei}</span> · {handoff.targetTechnicianName || 'KTV nhận'}</strong><span className="text-zinc-500">Từ {handoff.fromTechnicianName || 'KTV trước'} · {handoff.reason}</span></button>)}</div></div>}
