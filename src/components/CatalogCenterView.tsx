...
export const CatalogCenterView: React.FC<CatalogCenterViewProps> = ({ items: _items }) => {
  ...
  const [dictionaryDraft, setDictionaryDraft] = useState<{
    scope: CatalogDictionaryRecord['scope'];
    group: string;
    label: string;
    code: string;
    aliases: string;
  }>({ scope: 'CATEGORY', group: '', label: '', code: '', aliases: '' });
  ...
  const loadCatalogPage = useCallback(async (cursor?: string, append = false) => {
    ...
    try {
      ...
      if (requestId === catalogRequestId.current) setApiError(error?.message || 'Không thể tải Product Master từ máy chủ.');
    } finally {
  ...
  const loadBootstrap = async () => {
    ...
    try {
      ...
      setBootstrap({ models: result.models || [], dictionaries: result.dictionaries || [], itemSummary: result.itemSummary });
    } catch (error: any) {
      setApiError(error?.message || 'Không thể tải cấu hình Product Master từ máy chủ.');
    } finally {
  ...
  const dictionaryGroups = useMemo(() => {
    ...
    dictionaries.forEach(entry => {
      const label = entry.scope === 'CATEGORY'
        ? 'Nhóm & loại hàng'
        : entry.scope === 'BRAND'
          ? 'Thương hiệu / nhà sản xuất'
          : entry.group || 'Thuộc tính';
  ...
  const selectedModels = useMemo(() => models.filter(model => bulkModelIds.includes(model.id)), [models, bulkModelIds]);
  ...
  const validateBulkSetup = () => {
    if (!bulkCategoryName.trim() || !codeLabel(bulkCategoryCode)) return 'Chọn loại hàng đã được thiết lập trong Bộ từ điển trước khi tạo preview.';
    if (!dictionaryForScope('CATEGORY').some(entry => codeLabel(entry.code) === codeLabel(bulkCategoryCode))) {
      return 'SKU prefix phải lấy từ Bộ từ điển mã, không dùng giá trị tự gõ.';
    }
    if (!selectedModels.length) return 'Chọn ít nhất một Model Master.';
    if (!bulkUnit?.label) return 'Chọn đơn vị tính đã được thiết lập trong Bộ từ điển.';
    const missingModelCode = selectedModels.find(model => !codeLabel(model.modelCode));
    if (missingModelCode) return `Model “${missingModelCode.modelName}” chưa có modelCode. Hãy hoàn tất Model Master.`;
  ...
  const handleBulkCreate = async () => {
    ...
    try {
      ...
      showNotice(`Đã tạo ${result.created?.length || 0} Product Master. Các SKU trùng được bỏ qua an toàn.`);
      ...
      setApiError(error?.message || 'Không thể tạo Product Master.');
    } finally {
  ...
  const handleCreateModel = async (event: React.FormEvent) => {
    ...
    if (!dictionaryForScope('BRAND').some(entry => codeLabel(entry.code) === codeLabel(modelDraft.brandCode))) {
      showNotice('Tạo mã thương hiệu trong Bộ từ điển trước, sau đó mới tạo Model Master.');
      return;
    }
    setApiError(null);
    try {
      ...
      setBootstrap(current => current ? { ...current, models: [model, ...current.models] } : { models: [model], dictionaries: [] });
      setModelDraft({ brandName: '', brandCode: '', seriesName: '', modelName: '', modelCode: '', releaseYear: '', aliases: '' });
      setIsModelFormOpen(false);
      showNotice(`Đã tạo Model Master: ${model.modelName}`);
    } catch (error: any) {
      setApiError(error?.message || 'Không thể lưu Model Master.');
    }
  };
  ...
  const handleCreateDictionary = async (event: React.FormEvent) => {
    ...
    setApiError(null);
    try {
      const entry = await catalogApi.createDictionary({
      ...
      setBootstrap(current => current ? { ...current, dictionaries: [entry, ...current.dictionaries] } : { models: [], dictionaries: [entry] });
      setDictionaryDraft({ scope: 'CATEGORY', group: '', label: '', code: '', aliases: '' });
      setIsDictionaryFormOpen(false);
      showNotice(`Đã thêm mã từ điển: ${entry.label}`);
    } catch (error: any) {
      setApiError(error?.message || 'Không thể lưu Bộ từ điển mã.');
    }
  };

  const handleClonePreview = async () => {
    if (!cloneSourceId || !cloneTargetId || cloneSourceId === cloneTargetId) {
      showNotice('Chọn hai Model Master khác nhau để nhân bản.');
      return;
    }
  ...
  const handleCloneCreate = async () => {
    ...
    try {
      ...
      showNotice(`Đã nhân bản ${result.created?.length || 0} Product Master sang model mới.`);
  ...
  const handlePasteCreate = async () => {
    ...
    try {
      ...
      showNotice(`Đã nhập ${result.created?.length || 0} Product Master từ bảng.`);
  ...
  const handleRollbackLastOperation = async () => {
    ...
    const approved = window.confirm(
      `Hoàn tác lần ${lastOperation.label} này?\n\nHệ thống chỉ lưu trữ (archive) Product Master được tạo bởi operation ${lastOperation.operationKey}. Không xóa SKU registry, không thay đổi tồn kho, lô hàng, IMEI hay chứng từ. Những mã đã được liên kết tồn kho sẽ bị chặn.`
    );
    if (!approved) return;
    ...
    try {
      ...
      showNotice(`Đã hoàn tác ${result.summary.archived} Product Master${blockedHint}.`);
      setLastOperation(null);
      await loadBootstrap();
      await loadCatalogPage();
    } catch (error: any) {
      setApiError(error?.message || 'Không thể hoàn tác lần tạo Product Master này.');
    } finally {
    ...
    <div className="space-y-4 pb-24 text-zinc-900 animate-fadeIn">
      ...
      <section className="overflow-hidden rounded-3xl border border-orange-200 bg-gradient-to-br from-zinc-950 via-zinc-900 to-orange-950 text-white shadow-lg">
          ...
          <div className="relative flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-orange-300/30 bg-orange-400/10 px-3 py-1 text-[10px] font-black tracking-[0.16em] text-orange-200">
                <Database className="h-3.5 w-3.5" /> PRODUCT MASTER · KHÔNG PHẢI TỒN KHO
              </div>
              <h1 className="text-xl font-black tracking-tight sm:text-2xl">Trung tâm Danh mục &amp; Sinh mã hàng loạt</h1>
              <p className="mt-1 max-w-2xl text-xs leading-5 text-zinc-300 sm:text-sm">
                Khai báo Model Master và từ điển mã một lần. SKU được máy chủ sinh có kiểm tra trùng; tồn kho, lô hàng và IMEI vẫn vận hành độc lập.
              </p>
            </div>
        ...
        <div className="relative flex gap-1 overflow-x-auto border-t border-white/10 bg-black/10 px-2 py-2 sm:px-4">
          <TabButton tab="CATALOG" icon={Package}>Danh mục</TabButton>
          <TabButton tab="MODELS" icon={Layers}>Model &amp; từ điển</TabButton>
          <TabButton tab="BULK" icon={Sparkles}>Tạo hàng loạt</TabButton>
          <TabButton tab="TOOLS" icon={ClipboardPaste}>Nhân bản &amp; Paste Excel</TabButton>
        </div>
      </section>
      ...
      {lastOperation && (
        <div className="flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-start gap-2">
            ...
            <div>
              <p className="text-xs font-black text-amber-950">Lần tạo gần nhất: {lastOperation.createdCount} Product Master từ {lastOperation.label}</p>
              <p className="mt-0.5 text-[11px] leading-4 text-amber-800">Có thể hoàn tác an toàn theo operation <span className="font-mono font-bold">{lastOperation.operationKey}</span>. Chỉ archive Product Master chưa liên kết; không tác động tồn kho, IMEI, lô hoặc chứng từ.</p>
            </div>
          </div>
        ...
        <section className="space-y-3">
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3">
              <div>
                <h2 className="text-sm font-black text-zinc-800">Danh mục Product Master</h2>
              ...
              <span className="hidden rounded-lg bg-zinc-100 px-2 py-1 text-[10px] font-bold text-zinc-500 sm:inline-flex">Chỉ đọc · server-authoritative</span>
            </div>
            {catalogLoading ? (
              <div className="flex items-center justify-center gap-2 px-4 py-12 text-xs font-bold text-zinc-500"><RefreshCw className="h-4 w-4 animate-spin text-orange-500" /> Đang tải Product Master…</div>
            ) : catalogItems.length === 0 ? (
              <EmptyState icon={Package} title="Chưa có Product Master phù hợp" description="Tạo Model Master và Bộ từ điển, sau đó dùng Sinh mã hàng loạt để tạo SKU có kiểm tra trùng." action={<button type="button" onClick={() => setActiveTab('MODELS')} className="rounded-xl bg-zinc-950 px-3 py-2 text-xs font-bold text-white">Thiết lập dữ liệu gốc</button>} />
            ) : (
              <>
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          <div className="space-y-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2"><Layers className="h-4 w-4 text-orange-500" /><h2 className="text-sm font-black">Device Model Master</h2></div>
                <p className="mt-1 text-[11px] leading-5 text-zinc-500">Mỗi model chỉ khai báo một lần. Linh kiện, phụ kiện, báo giá và tìm kiếm sẽ tham chiếu đúng model này.</p>
              </div>
              <button type="button" disabled={loadingBootstrap || Boolean(apiError)} onClick={() => setIsModelFormOpen(true)} className="inline-flex shrink-0 items-center gap-1 rounded-xl bg-zinc-950 px-3 py-2 text-xs font-bold text-white hover:bg-black"><Plus className="h-3.5 w-3.5 text-orange-400" /> Thêm model</button>
            </div>
            {isModelFormOpen && (
              <form onSubmit={handleCreateModel} className="rounded-2xl border border-orange-200 bg-orange-50/50 p-3">
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  ...
                  <div><FieldLabel required hint="Không tự suy đoán từ tên model">Model code</FieldLabel><Input value={modelDraft.modelCode} onChange={event => setModelDraft(draft => ({ ...draft, modelCode: event.target.value }))} placeholder="Mã do admin tự đặt" /></div>
                ...
                <div className="mt-3 flex justify-end gap-2"><button type="button" onClick={() => setIsModelFormOpen(false)} className="rounded-xl px-3 py-2 text-xs font-bold text-zinc-600">Hủy</button><button type="submit" className="rounded-xl bg-orange-600 px-3 py-2 text-xs font-bold text-white">Lưu Model Master</button></div>
              </form>
            )}
            {loadingBootstrap ? <div className="py-10 text-center text-xs text-zinc-400">Đang tải Model Master…</div> : models.length === 0 ? <EmptyState icon={Layers} title="Chưa có model nào" description="Bắt đầu bằng model và mã chuẩn do quản trị viên đặt. Hệ thống không tự nạp danh sách iPhone hay mã giả định." /> : (
              <div className="max-h-[510px] overflow-y-auto rounded-xl border border-zinc-100">
                {models.map(model => (
                  <div key={model.id} className="flex items-center justify-between gap-3 border-b border-zinc-100 px-3 py-3 last:border-0">
                    <div className="min-w-0"><p className="truncate text-xs font-black text-zinc-800">{model.modelName}</p><p className="mt-0.5 text-[10px] text-zinc-500">{model.brandName}{model.seriesName ? ` · ${model.seriesName}` : ''}{model.releaseYear ? ` · ${model.releaseYear}` : ''}</p><div className="mt-1 flex flex-wrap gap-1">{(model.aliases || []).slice(0, 3).map(alias => <span key={alias} className="rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] text-zinc-500">{alias}</span>)}</div></div>
                    <div className="shrink-0 text-right"><CodeBadge code={model.modelCode} /><div className="mt-1"><CodeBadge code={model.brandCode} muted={!model.brandCode} /></div></div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="space-y-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2"><Tags className="h-4 w-4 text-orange-500" /><h2 className="text-sm font-black">Bộ từ điển mã</h2></div>
                <p className="mt-1 text-[11px] leading-5 text-zinc-500">Mã loại hàng, hãng và thuộc tính là cấu hình quản trị. Engine chỉ ghép các mã này theo quy tắc; không dùng AI hoặc chuỗi ngẫu nhiên.</p>
              </div>
              <button type="button" disabled={loadingBootstrap || Boolean(apiError)} onClick={() => setIsDictionaryFormOpen(true)} className="inline-flex shrink-0 items-center gap-1 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-xs font-bold text-zinc-700 hover:bg-zinc-50"><Plus className="h-3.5 w-3.5 text-orange-500" /> Thêm mã</button>
            </div>
            {isDictionaryFormOpen && (
              <form onSubmit={handleCreateDictionary} className="rounded-2xl border border-orange-200 bg-orange-50/50 p-3">
                <div className="mb-2 flex items-center justify-between"><p className="text-xs font-black text-orange-900">Thêm giá trị từ điển</p><button type="button" onClick={() => setIsDictionaryFormOpen(false)} className="rounded-lg p-1 text-zinc-500 hover:bg-white"><X className="h-4 w-4" /></button></div>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  <div><FieldLabel required>Nhóm từ điển</FieldLabel><Select value={dictionaryDraft.scope} onChange={event => setDictionaryDraft(draft => ({ ...draft, scope: event.target.value as CatalogDictionaryRecord['scope'] }))}><option value="CATEGORY">Loại hàng</option><option value="BRAND">Thương hiệu / nhà SX</option><option value="ATTRIBUTE">Thuộc tính biến thể</option></Select></div>
                  <div><FieldLabel hint="Dùng để phân nhóm thuộc tính, không phải SKU">Nhóm thuộc tính</FieldLabel><Input value={dictionaryDraft.group} onChange={event => setDictionaryDraft(draft => ({ ...draft, group: event.target.value }))} placeholder={dictionaryDraft.scope === 'ATTRIBUTE' ? 'Ví dụ: QUALITY, COLOR...' : 'Không bắt buộc'} /></div>
                  ...
                  <div><FieldLabel required>Mã chuẩn</FieldLabel><Input value={dictionaryDraft.code} onChange={event => setDictionaryDraft(draft => ({ ...draft, code: event.target.value }))} placeholder="Mã do admin định nghĩa" /></div>
                ...
                <div className="mt-3 flex justify-end gap-2"><button type="button" onClick={() => setIsDictionaryFormOpen(false)} className="rounded-xl px-3 py-2 text-xs font-bold text-zinc-600">Hủy</button><button type="submit" className="rounded-xl bg-orange-600 px-3 py-2 text-xs font-bold text-white">Lưu mã</button></div>
              </form>
            )}
            {loadingBootstrap ? <div className="py-10 text-center text-xs text-zinc-400">Đang tải Bộ từ điển…</div> : dictionaryGroups.length === 0 ? <EmptyState icon={Tags} title="Chưa có mã được cấu hình" description="Hãy tạo mã loại hàng, thương hiệu và các thuộc tính cần dùng. Không có bảng mã mẫu được tạo tự động trong code." /> : (
              <div className="max-h-[510px] space-y-3 overflow-y-auto">
                {dictionaryGroups.map(([group, entries]) => <div key={group} className="rounded-xl border border-zinc-100"><div className="border-b border-zinc-100 bg-zinc-50 px-3 py-2 text-[10px] font-black uppercase tracking-wide text-zinc-500">{group}</div>{entries.map(entry => <div key={entry.id} className="flex items-center justify-between gap-3 px-3 py-2.5"><div className="min-w-0"><p className="truncate text-xs font-bold text-zinc-700">{entry.label}</p>{entry.aliases?.length ? <p className="truncate text-[10px] text-zinc-400">{entry.aliases.join(' · ')}</p> : null}</div><CodeBadge code={entry.code} /></div>)}</div>)}
              </div>
            )}
          </div>
        </section>
      ...
      {activeTab === 'BULK' && (
        <section className="space-y-4">
          <div className="rounded-2xl border border-orange-200 bg-orange-50/60 p-4 text-xs leading-5 text-orange-900">
            <div className="flex items-start gap-2"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-orange-600" /><div><b>Ma trận biến thể có kiểm soát.</b> Chọn thuộc tính và model; hệ thống gửi tổ hợp lên server để sinh SKU deterministic, kiểm tra trùng trước khi cho tạo. Tạo Product Master không tạo số lượng tồn, không tạo IMEI và không cấp giá vốn.</div></div>
          </div>
            ...
            <div className="space-y-4 xl:col-span-5">
              <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
                ...
                <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                  <div><FieldLabel required>Loại hàng / SKU prefix</FieldLabel><Select value={bulkCategoryCode} onChange={event => { const selected = dictionaryForScope('CATEGORY').find(entry => codeLabel(entry.code) === event.target.value); setBulkCategoryCode(selected?.code || ''); setBulkCategoryName(selected?.label || ''); setPreview(null); }}><option value="">Chọn từ Bộ từ điển mã</option>{dictionaryForScope('CATEGORY').map(entry => <option key={entry.id} value={codeLabel(entry.code)}>{entry.label} · {entry.code}</option>)}</Select></div>
                  <div><FieldLabel required hint="Mã prefix do admin thiết lập trong Bộ từ điển; engine ghép vào đầu SKU.">Mã loại hàng</FieldLabel><Input readOnly value={bulkCategoryCode} placeholder="Chọn loại hàng bên trái" className="bg-zinc-50 font-mono text-zinc-600" /></div>
                </div>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
                <div className="mb-2 flex items-center justify-between"><div className="flex items-center gap-2"><Layers className="h-4 w-4 text-orange-500" /><h2 className="text-sm font-black">2. Chọn Model Master</h2></div><span className="rounded-lg bg-zinc-100 px-2 py-1 text-[10px] font-bold text-zinc-500">{bulkModelIds.length} đã chọn</span></div>
                {!models.length ? <EmptyState icon={Layers} title="Cần Model Master trước" description="Không thể tạo SKU từ tên tự gõ. Vào Model & từ điển để thiết lập modelCode." action={<button type="button" onClick={() => setActiveTab('MODELS')} className="rounded-xl bg-zinc-950 px-3 py-2 text-xs font-bold text-white">Mở Model Master</button>} /> : <div className="max-h-56 space-y-1 overflow-y-auto rounded-xl border border-zinc-100 p-1.5">{models.map(model => <button key={model.id} type="button" onClick={() => toggleBulkModel(model.id)} className={`flex w-full items-center justify-between gap-2 rounded-lg px-2.5 py-2 text-left transition ${bulkModelIds.includes(model.id) ? 'bg-orange-50 text-orange-900' : 'hover:bg-zinc-50'}`}><div className="min-w-0"><p className="truncate text-xs font-bold">{model.modelName}</p><p className="text-[10px] text-zinc-500">{model.brandName}{model.seriesName ? ` · ${model.seriesName}` : ''}</p></div><div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md border ${bulkModelIds.includes(model.id) ? 'border-orange-500 bg-orange-600 text-white' : 'border-zinc-300 bg-white text-transparent'}`}><Check className="h-3.5 w-3.5" /></div></button>)}</div>}
              </div>
            </div>
      ...
      {activeTab === 'TOOLS' && (
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          ...
          <div className="space-y-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
            <div className="flex items-center gap-2"><ClipboardPaste className="h-4 w-4 text-orange-500" /><div><h2 className="text-sm font-black">Paste Excel có kiểm tra</h2><p className="mt-0.5 text-[11px] text-zinc-500">Dán bảng tab-separated. Server chuẩn hóa, đối chiếu Model Master và từ điển rồi mới cho nhập.</p></div></div>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-orange-200 bg-white shadow-sm">
      ...
      <div className="flex flex-col gap-2 border-t border-zinc-100 p-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-[10px] leading-4 text-zinc-500">Máy chủ vẫn kiểm tra SKU và cấu hình một lần nữa trước khi ghi Product Master.</p>
