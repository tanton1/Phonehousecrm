...
> = ({ task, warehouses, funds = [], currentUser, onClose, onRefresh }) => {
  ...
  const usesCustomerPortalOtp = Boolean(workOrder.customerAccountUid);
  const customerDeliveryPanel = (
    <div className="mx-auto max-w-xl space-y-4">
      <section className="rounded-2xl border bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div><h3 className="font-black">Báo giá khách hàng</h3><p className="mt-1 text-xs text-zinc-500">{usesCustomerPortalOtp ? 'Quản lý duyệt trước, sau đó khách xác nhận phiên bản giá bằng OTP trên PhoneHouse Care.' : 'Giá chỉ có hiệu lực sau khi quản lý hoặc kế toán duyệt.'}</p></div>
        ...
        {canRequestQuote && workOrder.workOrderType !== "WARRANTY" && (
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            {usesCustomerPortalOtp ? <div className="rounded-xl bg-sky-50 p-3 text-xs font-bold leading-5 text-sky-800">Không cần nhập mã bằng chứng. Sau khi quản lý duyệt, khách sẽ nhận thông báo và xác nhận OTP.</div> : <label><span className="text-xs font-black text-zinc-600">Mã bằng chứng khách duyệt</span><input value={quoteRequest.customerApprovalEvidenceId} onChange={(event) => setQuoteRequest((current) => ({ ...current, customerApprovalEvidenceId: event.target.value }))} placeholder="Tin nhắn / chữ ký / evidence ID" className="mt-1 h-11 w-full rounded-xl border px-3" /></label>}
        ...
        {!!details?.quoteAdjustments?.length && <div className="mt-4 divide-y rounded-xl border">{details.quoteAdjustments.map((adjustment: any) => <div key={adjustment.id} className="p-3"><div className="flex items-center justify-between gap-3"><div><p className="font-bold">{money.format(Number(adjustment.requestedAmount || 0))}</p><p className="text-xs text-zinc-500">{adjustment.reason}</p></div><span className="text-xs font-black">{adjustment.status}</span></div>{canApproveQuote && adjustment.status === "PENDING" && <div className="mt-2 flex gap-2"><button disabled={saving || (!usesCustomerPortalOtp && Number(adjustment.requestedAmount || 0) > 0 && !adjustment.customerApprovalEvidenceId)} onClick={() => void run(() => decideQuoteAdjustment(adjustment.id, "APPROVED"), usesCustomerPortalOtp ? "Đã duyệt nội bộ. Đang chờ khách xác nhận OTP." : "Đã duyệt giá cuối cùng.")} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-black text-white disabled:opacity-40">Duyệt</button><button disabled={saving} onClick={() => void run(() => decideQuoteAdjustment(adjustment.id, "REJECTED"), "Đã từ chối báo giá.")} className="rounded-lg bg-red-50 px-3 py-1.5 text-xs font-black text-red-700">Từ chối</button></div>}</div>)}</div>}
      </section>
      <section className="rounded-2xl border bg-white p-5">
