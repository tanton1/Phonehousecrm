...
export const TechnicalWorkOrderDrawer: React.FC<TechnicalWorkOrderDrawerProps> = ({ task, warehouses, funds = [], currentUser, onClose, onRefresh }) => {
  ...
  const completeSelectedTask = async () => {
    ...
    const urls = completionFiles.length ? await uploadTechnicalEvidence(workOrderId, selectedLine.id, completionFiles) : [];
  ...
  const requestHandoff = async () => {
    ...
    const handoverPhotoUrls = handoffFiles.length ? await uploadTechnicalEvidence(workOrderId, 'handoff-request', handoffFiles) : [];
    ...
    <aside className="flex h-full min-w-0 w-full max-w-5xl flex-col bg-zinc-50 shadow-2xl">
      ...
      <main className="min-w-0 flex-1 overflow-x-hidden overflow-y-auto p-3 sm:p-6">
        ...
        {loading && !details ? <div className="grid h-48 place-items-center"><Loader2 className="h-8 w-8 animate-spin text-orange-600"/></div> : <>
          ...
          {activeTab === 'OVERVIEW' && canRequestHandoff && <section className="mt-4 rounded-2xl border border-blue-200 bg-blue-50/40 p-5"><h3 className="font-black text-zinc-900">Bàn giao trách nhiệm sang KTV khác</h3><p className="mt-1 text-xs text-zinc-500">Task đang chạy phải dừng, linh kiện phải đối soát. Trách nhiệm chỉ đổi sau khi KTV nhận quét đúng IMEI; ảnh là tùy chọn.</p>{workOrder.activeHandoffId ? <div className="mt-3 rounded-xl bg-amber-100 p-3 text-sm font-bold text-amber-800">Đang chờ KTV đích xác nhận bàn giao: {workOrder.activeHandoffId}</div> : <><div className="mt-4 grid gap-3 sm:grid-cols-2"><select value={handoffTargetWarehouseId} onChange={event => setHandoffTargetWarehouseId(event.target.value)} className="h-11 rounded-xl border bg-white px-3 text-sm"><option value="">Chọn kho/KTV nhận</option>{eligibleHandoffWarehouses.map(item => <option key={item.id} value={item.id}>{item.name} · {item.custodianName || item.technicianName}</option>)}</select><input value={handoffScannedImei} onChange={event => setHandoffScannedImei(event.target.value.replace(/\D/g, '').slice(0, 15))} placeholder="Quét IMEI bàn giao" className="h-11 rounded-xl border px-3 font-mono text-sm"/><input value={handoffReason} onChange={event => setHandoffReason(event.target.value)} placeholder="Lý do bàn giao" className="h-11 rounded-xl border px-3 text-sm sm:col-span-2"/><label className="rounded-xl border border-dashed bg-white p-3 text-xs font-bold sm:col-span-2">Ảnh tình trạng lúc bàn giao (không bắt buộc)<input type="file" accept="image/*" multiple onChange={event => setHandoffFiles(Array.from(event.target.files || []))} className="mt-2 block w-full text-xs"/></label></div><button disabled={saving || !handoffTargetWarehouseId || !handoffScannedImei || handoffReason.trim().length < 5} onClick={() => void run(requestHandoff, 'Đã tạo yêu cầu; trách nhiệm vẫn thuộc KTV hiện tại cho đến khi người nhận xác nhận.')} className="mt-3 rounded-xl bg-blue-700 px-4 py-2.5 text-sm font-black text-white disabled:opacity-40">Tạo yêu cầu bàn giao</button></>}</section>}
          ...
          {activeTab === 'TASKS' && <div className="space-y-4">
            ...
            {selectedLine && String(selectedLine.status || '') === 'IN_PROGRESS' && <section className="rounded-2xl border bg-white p-5"><h3 className="font-black">Báo hoàn thành: {selectedLine.taskName}</h3><p className="mt-1 text-xs text-zinc-500">Ảnh là tùy chọn. Trước khi hoàn thành, linh kiện đã giữ/xuất vẫn phải được xác nhận dùng, trả hoặc báo hỏng.</p><textarea value={completionNotes} onChange={event => setCompletionNotes(event.target.value)} rows={3} placeholder="Mô tả kết quả trước/sau, thông số thay đổi..." className="mt-3 w-full rounded-xl border p-3 text-sm"/>{Array.isArray(selectedLine.requiredEvidenceTypes) && selectedLine.requiredEvidenceTypes.includes('REPLACEMENT_SERIAL') && <textarea value={replacementSerials} onChange={event => setReplacementSerials(event.target.value)} rows={2} placeholder="Serial linh kiện thay thế, mỗi serial một dòng" className="mt-3 w-full rounded-xl border p-3 font-mono text-sm"/>}<label className="mt-3 block rounded-xl border border-dashed p-4 text-sm"><span className="font-bold">Ảnh bằng chứng trước/sau (không bắt buộc)</span><input type="file" accept="image/*" multiple onChange={event => setCompletionFiles(Array.from(event.target.files || []))} className="mt-2 block w-full text-xs"/><span className="mt-1 block text-xs text-zinc-500">Đã chọn {completionFiles.length} ảnh, tối đa 8 ảnh · 20MB/ảnh.</span></label><button disabled={saving} onClick={() => void run(completeSelectedTask, result => result?.allLinesCompleted ? 'Đã hoàn thành toàn bộ task; phiếu chuyển sang chờ KCS.' : 'Đã hoàn thành task này. Các task còn lại vẫn tiếp tục xử lý.')} className="mt-4 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-black text-white disabled:opacity-50">Hoàn thành hạng mục</button></section>}
