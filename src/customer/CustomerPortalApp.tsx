...
import {
  ...
  requestCustomerPushToken,
  resetCustomerPhoneRecaptcha,
} from "../lib/firebase";
import {
  CUSTOMER_REPAIR_ISSUES,
  customerRepairIssueByCode,
} from "../../shared/customerRepairIssues";
import {
...
type Overlay =
  ...
  | "promotion"
  | null;
const QuickQuoteMiniweb = React.lazy(() =>
  import("./QuickQuoteMiniweb").then((module) => ({
    default: module.QuickQuoteMiniweb,
  })),
);

const portalPaths: Record<PortalTab, string> = {
  home: "/khach-hang",
  devices: "/khach-hang/thiet-bi",
  repairs: "/khach-hang/sua-chua",
  promotions: "/khach-hang/uu-dai",
  account: "/khach-hang/tai-khoan",
};
...
function StatusPill({
  ...
  label: string;
  tone?: "orange" | "green" | "blue" | "gray" | "red";
}) {
  const tones = {
...
function AppButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={classNames(
        "min-h-11 rounded-2xl px-4 py-2.5 text-sm font-black transition disabled:cursor-not-allowed disabled:opacity-50",
        primary
          ? "bg-[#ff4b16] text-white shadow-lg shadow-orange-200 hover:bg-[#e94312]"
          : "border border-zinc-200 bg-white text-zinc-700 hover:border-orange-300 hover:text-orange-700",
        className,
      )}
    >
      {children}
...
function EmptyState({
  ...
  return (
    <div className="rounded-3xl border border-dashed border-zinc-200 bg-white px-5 py-10 text-center">
      <Icon className="mx-auto h-9 w-9 text-zinc-300" />
      <p className="mt-3 font-black text-zinc-800">{title}</p>
      <p className="mx-auto mt-1 max-w-sm text-sm text-zinc-500">{text}</p>
...
function CustomerLoginView({
  ...
  const [confirmation, setConfirmation] = useState<Awaited<
    ReturnType<typeof requestCustomerPhoneOtp>
  > | null>(null);
  ...
  const sendCode = async () => {
    ...
    setError("");
    try {
      const result = await requestCustomerPhoneOtp(
        phone,
  ...
  const verify = async () => {
    ...
    try {
      ...
      try {
        ...
        if (
          String(cause?.message || "").includes(
            "CUSTOMER_IDENTITY_ADDITIONAL_VERIFICATION_REQUIRED",
          )
        )
          setNeedsLink(true);
        else throw cause;
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Đăng nhập PhoneHouse Care"
      className="fixed inset-0 z-[100] overflow-y-auto bg-[#fffaf7] sm:bg-black/50 sm:p-6"
    >
      <div className="mx-auto min-h-full w-full max-w-lg bg-[#fffaf7] sm:min-h-0 sm:rounded-[2rem] sm:shadow-2xl">
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-orange-100 bg-[#fffaf7]/95 px-4 py-3 backdrop-blur sm:rounded-t-[2rem]">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#ff4b16] text-white">
              <Smartphone className="h-5 w-5" />
            </div>
            <div>
              <p className="text-sm font-black">PhoneHouse Care</p>
              <p className="text-[11px] font-semibold text-zinc-500">
                Đăng nhập an toàn bằng OTP
              </p>
            </div>
          </div>
          <button
            ...
            onClick={onClose}
            className="flex h-11 w-11 items-center justify-center rounded-2xl text-zinc-600 hover:bg-zinc-100"
          >
            <X className="h-5 w-5" />
          </button>
        </header>
        ...
        <div className="px-4 pb-8 pt-5 sm:px-7 sm:pb-7">
          <div className="mb-5">
            <p className="text-xs font-black uppercase tracking-[.18em] text-[#ff4b16]">
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-500">
              Tra cứu đúng bảo hành, phiếu sửa và tiến độ gắn với số điện thoại
              đã mua hàng.
            </p>
          </div>
            ...
            {needsLink ? (
              <>
                <p className="text-sm leading-6 text-zinc-600">
                  Có nhiều hồ sơ dùng số này. Nhập mã hóa đơn hoặc IMEI đã mua
                  để PhoneHouse liên kết đúng dữ liệu.
                </p>
                <label className="mt-4 block text-sm font-bold text-zinc-700">
                  Mã hóa đơn hoặc IMEI
                  <input
                    autoFocus
                    ...
                    className="mt-1.5 h-12 w-full rounded-2xl border border-zinc-200 px-3 font-mono outline-none focus:border-orange-500"
                    placeholder="VD: HD-260901 hoặc 35…"
                  />
                </label>
                <AppButton
                  primary
              ...
              <>
                <p className="text-sm leading-6 text-zinc-600">
                  Mã xác nhận đã được gửi. Nhập 6 số trong tin nhắn để tiếp tục.
                </p>
                <input
                  autoFocus
                  ...
                  className="mt-4 h-14 w-full rounded-2xl border border-zinc-200 text-center text-2xl font-black tracking-[.35em] outline-none focus:border-orange-500"
                  placeholder="••••••"
                />
                <AppButton
                  primary
                ...
                <button
                  type="button"
                  className="mt-3 min-h-11 w-full text-xs font-bold text-zinc-500 underline"
                  onClick={() => {
              ...
              <>
                <label className="block text-sm font-bold text-zinc-700">
                  ...
                  <input
                    ...
                    className="mt-1.5 h-12 w-full rounded-2xl border border-zinc-200 px-3 text-base outline-none focus:border-orange-500"
                ...
                <p className="mt-3 text-center text-[10px] leading-4 text-zinc-400">
                  Firebase sẽ mở bước xác minh chống spam khi cần. reCAPTCHA áp
                  dụng Chính sách quyền riêng tư và Điều khoản của Google.
                </p>
              </>
            )}
            {error && (
              <p
                role="alert"
                className="mt-3 rounded-xl bg-rose-50 p-3 text-xs font-bold leading-5 text-rose-700"
              >
                {error}
              </p>
            )}
          ...
          <button
            type="button"
            onClick={onClose}
            className="mt-4 min-h-11 w-full text-center text-sm font-bold text-zinc-600 underline underline-offset-4"
          >
            Tiếp tục xem ưu đãi và cửa hàng
          </button>
        </div>
      </div>
    </div>
...
function BottomNav({
  ...
  const items: Array<[PortalTab, string, React.ElementType]> = [
    ["home", "Trang chủ", Smartphone],
    ["devices", "Thiết bị", ShieldCheck],
    ["repairs", "Sửa chữa", Wrench],
    ["promotions", "Ưu đãi", Sparkles],
    ["account", "Tài khoản", UserRound],
  ];
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-zinc-200 bg-white/95 px-1 pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_30px_rgba(0,0,0,.06)] backdrop-blur lg:sticky lg:bottom-auto lg:mt-8 lg:rounded-2xl lg:border lg:p-2 lg:shadow-none">
      <div className="mx-auto flex max-w-2xl justify-around lg:gap-2">
        {items.map(([id, label, Icon]) => (
          <button
            key={id}
            onClick={() => onChange(id)}
            className={classNames(
              "flex min-h-14 min-w-[62px] flex-1 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-black transition lg:min-h-11 lg:flex-row lg:gap-2 lg:text-xs",
              active === id
                ? "bg-orange-50 text-[#ff4b16]"
...
function Header({
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-zinc-100 bg-[#fffaf7]/95 px-4 py-3 backdrop-blur sm:px-6">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#ff4b16] text-white">
            <Smartphone className="h-5 w-5" />
          </div>
          <div className="min-w-0">
            <p className="truncate text-sm font-black text-zinc-950">
              {brand.name || "PhoneHouse"}{" "}
              <span className="font-medium text-zinc-400">Care</span>
            </p>
            <p className="truncate text-[11px] font-semibold text-zinc-500">
              {me ? `Xin chào, ${me.displayName}` : brand.slogan}
            </p>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-1">
          {me ? (
            <button
              ...
              className="relative flex h-11 w-11 items-center justify-center rounded-2xl text-zinc-700 hover:bg-zinc-100"
            >
              <Bell className="h-5 w-5" />
              {unread > 0 && (
                <span className="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#ff4b16] px-1 text-[9px] font-black text-white">
                  {unread > 9 ? "9+" : unread}
                </span>
              )}
            </button>
            ...
            <button
              onClick={onLogin}
              className="flex h-11 items-center gap-1.5 rounded-2xl bg-zinc-950 px-3 text-xs font-black text-white"
            >
...
function HomePage({
  ...
  onOpenQuote: () => void;
  onOpenChat: () => void;
  onOpenDevice: (device: CustomerDevice) => void;
}) {
  ...
  return (
    <div className="space-y-5">
      <section className="relative overflow-hidden rounded-[2rem] bg-zinc-950 p-5 text-white shadow-xl sm:p-7">
        <div className="absolute -right-16 -top-20 h-56 w-56 rounded-full bg-[#ff4b16]/30 blur-3xl" />
        <div className="relative max-w-xl">
          ...
          <h1 className="mt-2 text-2xl font-black leading-tight sm:text-3xl">
            An tâm dùng máy,
            <br />
            có PhoneHouse đồng hành.
          </h1>
          <p className="mt-3 max-w-md text-sm leading-6 text-zinc-300">
            Theo dõi bảo hành, sửa chữa và nhận hỗ trợ minh bạch ngay trên điện
            thoại.
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            <AppButton primary onClick={onOpenQuote}>
        ...
        <section>
          <div className="mb-2 flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs font-black text-[#ff4b16]"
            >
              Xem tất cả
            </button>
          </div>
          <button
            onClick={() => onTab("repairs")}
            className="w-full rounded-3xl border border-orange-100 bg-white p-4 text-left shadow-sm transition hover:border-orange-300"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-black text-zinc-900">
                  {activeRepairs[0].model}
                </p>
                <p className="mt-1 font-mono text-xs text-zinc-500">
                  {activeRepairs[0].imeiMasked} · {activeRepairs[0].code}
                </p>
              </div>
              <StatusPill
                label={activeRepairs[0].stageLabel}
                tone={
                  activeRepairs[0].stage === "WAITING_QUOTE_APPROVAL"
                    ? "orange"
                    : "blue"
                }
              />
            ...
            <div className="mt-4 flex items-center justify-between text-xs">
              <span className="text-zinc-500">
                {activeRepairs[0].promisedAt
                  ? `Hẹn trả ${dateTime(activeRepairs[0].promisedAt)}`
                  : "Đang cập nhật thời gian hẹn"}
              </span>
      ...
      {me && expiring.length > 0 && (
        <section>
          <h2 className="mb-2 text-base font-black text-zinc-950">
            Bảo hành sắp hết
          </h2>
          <div className="flex gap-3 overflow-x-auto pb-1">
            {expiring.slice(0, 3).map((device) => (
              <button
                key={device.id}
                onClick={() => onOpenDevice(device)}
                className="min-w-[240px] rounded-3xl border border-amber-100 bg-amber-50 p-4 text-left"
              >
                <div className="flex items-center gap-3">
      ...
      <section>
        <div className="mb-2 flex items-center justify-between">
          <h2 className="text-base font-black text-zinc-950">Khám phá nhanh</h2>
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <button
            onClick={() => onTab("devices")}
            className="rounded-3xl border border-zinc-200 bg-white p-4 text-left"
          >
            <ShieldCheck className="h-6 w-6 text-emerald-600" />
            <p className="mt-3 text-sm font-black">Bảo hành</p>
            <p className="mt-1 text-xs text-zinc-500">Kiểm tra thiết bị</p>
          </button>
          <button
          ...
          >
            ...
            <p className="mt-3 text-sm font-black">Sửa chữa</p>
            <p className="mt-1 text-xs text-zinc-500">Gửi yêu cầu online</p>
          </button>
          <button
            onClick={() => onTab("promotions")}
            className="rounded-3xl border border-zinc-200 bg-white p-4 text-left"
          >
            <Sparkles className="h-6 w-6 text-purple-600" />
            <p className="mt-3 text-sm font-black">Ưu đãi</p>
            <p className="mt-1 text-xs text-zinc-500">Dành cho bạn</p>
          </button>
          <a
            href={`tel:${bootstrap.brand.hotline}`}
            className="rounded-3xl border border-zinc-200 bg-white p-4 text-left"
          >
            <Phone className="h-6 w-6 text-sky-600" />
            <p className="mt-3 text-sm font-black">Hotline</p>
            <p className="mt-1 text-xs text-zinc-500">
              {bootstrap.brand.hotline || "Liên hệ ngay"}
            </p>
          </a>
        </div>
      </section>
      <PromotionStrip
        ...
        onOpenPromotion={() => onTab("promotions")}
      />
    </div>
  );
}
...
function PromotionStrip({
  ...
  return (
    <section>
      <div className="mb-2 flex items-center justify-between">
        <h2 className="text-base font-black text-zinc-950">Ưu đãi nổi bật</h2>
        <button
      ...
      <div className="flex gap-3 overflow-x-auto pb-1">
        {promotions.slice(0, 3).map((promotion) => (
          <button
            key={promotion.id}
            onClick={onOpenPromotion}
            className="min-w-[270px] overflow-hidden rounded-3xl border border-zinc-200 bg-white text-left shadow-sm"
          >
            <div className="h-24 bg-gradient-to-br from-orange-100 via-amber-50 to-white p-4">
              {promotion.bannerUrl ? (
                <img
            ...
            <div className="p-4">
              <div className="flex items-center justify-between gap-2">
                <p className="line-clamp-1 font-black text-zinc-900">
                  {promotion.title}
                </p>
                {promotion.personalized && (
                  <StatusPill label="Dành cho bạn" tone="orange" />
                )}
              </div>
...
function DevicesPage({
  ...
  devices: CustomerDevice[];
  onOpen: (device: CustomerDevice) => void;
  onLogin: () => void;
}) {
  if (!devices.length)
    return (
      <EmptyState
        icon={ShieldCheck}
        title="Chưa có thiết bị được liên kết"
        text="Đăng nhập bằng số điện thoại đã mua hàng tại PhoneHouse để xem bảo hành và lịch sử thiết bị."
        action={
          <AppButton primary onClick={onLogin}>
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Thông tin được liên kết theo số điện thoại đã xác minh.
        </p>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        {devices.map((device) => (
          <button
            ...
            className="rounded-3xl border border-zinc-200 bg-white p-4 text-left shadow-sm transition hover:border-orange-300"
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="truncate font-black text-zinc-900">
                    {device.model || "Thiết bị PhoneHouse"}
                  </p>
                  <p className="mt-1 font-mono text-xs text-zinc-500">
                    {device.imeiMasked}
                  </p>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 border-t border-zinc-100 pt-3 text-xs">
              <div>
                <p className="text-zinc-500">Hết hạn</p>
                <p className="mt-1 font-bold text-zinc-800">
                  {dateOnly(device.warrantyUntil)}
                </p>
              </div>
              <div>
                <p className="text-zinc-500">Chi nhánh</p>
                <p className="mt-1 truncate font-bold text-zinc-800">
                  {device.branchName || "PhoneHouse"}
                </p>
              </div>
            </div>
          </button>
...
function RepairsPage({
  ...
  onOpen: (repair: CustomerRepair) => void;
  onRequest: () => void;
  onLogin: () => void;
}) {
  if (!repairs.length && !requests.length)
    return (
      <EmptyState
        icon={Wrench}
        title="Chưa có lịch sử sửa chữa"
        text="Bạn có thể gửi yêu cầu sửa chữa hoặc bảo hành trực tiếp trên app."
        action={
          <AppButton primary onClick={onRequest}>
  ...
  return (
    <div className="space-y-5">
      ...
      {requests.length > 0 && (
        <section>
          <h2 className="mb-2 text-sm font-black text-zinc-700">
            Yêu cầu đã gửi
          </h2>
          <div className="space-y-2">
            {requests.map((request) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-black text-zinc-900">{request.model}</p>
                    <p className="mt-1 text-xs text-zinc-500">
                ...
                <p className="mt-2 text-[11px] text-zinc-400">
                  Gửi lúc {dateTime(request.createdAt)} · {request.branchName}
                </p>
              </div>
            ))}
          </div>
      ...
      <section>
        ...
        {repairs.length ? (
          <div className="space-y-3">
            {repairs.map((repair) => (
              <button
                ...
                className="w-full rounded-3xl border border-zinc-200 bg-white p-4 text-left shadow-sm transition hover:border-orange-300"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-black text-zinc-900">{repair.model}</p>
                    <p className="mt-1 font-mono text-xs text-zinc-500">
                      {repair.imeiMasked} · {repair.code}
                    </p>
                  </div>
                  <StatusPill
                    label={repair.stageLabel}
                    tone={
                      repair.stage === "COMPLETED"
                        ? "green"
                        : repair.stage === "WAITING_QUOTE_APPROVAL"
                          ? "orange"
                          : "blue"
                    }
                  />
                ...
                <div className="mt-3 flex items-center justify-between text-xs">
                  <span className="text-zinc-500">
                    {repair.promisedAt
                      ? `Hẹn trả: ${dateTime(repair.promisedAt)}`
                      : "Đang cập nhật hẹn trả"}
                  </span>
                  <ChevronRight className="h-4 w-4 text-zinc-400" />
                </div>
              </button>
          ...
          <p className="text-sm text-zinc-500">
            Chưa có Work Order chính thức. Nhân viên sẽ cập nhật sau khi tiếp
            nhận yêu cầu.
          </p>
        )}
      </section>
...
function PromotionsPage({
  ...
  promotions: CustomerPromotion[];
  onOpen: (promotion: CustomerPromotion) => void;
}) {
  ...
  const items =
    filter === "ALL"
      ? promotions
      : promotions.filter((item) => item.category === filter);
  return (
    <div className="space-y-4">
      <div>
        <p className="text-xs font-black uppercase tracking-[.16em] text-[#ff4b16]">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Điều kiện và thời hạn được cập nhật từ hệ thống.
        </p>
      </div>
      ...
      {items.length ? (
        <div className="grid gap-3 sm:grid-cols-2">
          {items.map((promotion) => (
            <button
              ...
              className="overflow-hidden rounded-3xl border border-zinc-200 bg-white text-left shadow-sm transition hover:border-orange-300"
            >
              <div className="h-36 bg-gradient-to-br from-orange-100 via-amber-50 to-white p-4">
                {promotion.bannerUrl ? (
                  <img
                    src={promotion.bannerUrl}
                    alt=""
              ...
              <div className="p-4">
                <div className="flex flex-wrap items-center gap-2">
                  <StatusPill
                    label={
                      promotion.category === "REPAIR"
                        ? "Sửa chữa"
                        : promotion.category === "DEVICE"
                          ? "Mua máy"
                          : promotion.category === "ACCESSORY"
                            ? "Phụ kiện"
                  ...
                  />
                  {promotion.personalized && (
                    <StatusPill label="Dành cho bạn" tone="green" />
                  )}
                </div>
                <h2 className="mt-3 font-black text-zinc-900">
                  {promotion.title}
                </h2>
        ...
        <EmptyState
          ...
          text="Hãy quay lại sau hoặc bật nhận thông tin khuyến mãi trong Tài khoản."
        />
      )}
    </div>
  );
...
function AccountPage({
  ...
  me: CustomerMe | null;
  onLogin: () => void;
  onSave: (input: Partial<CustomerMe>) => Promise<void>;
  onLogout: () => void;
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useState(
    me?.notificationConsent !== false,
  );
  const [pushStatus, setPushStatus] = useState("");
  const [pushBusy, setPushBusy] = useState(false);
  useEffect(() => {
    setName(me?.displayName || "");
    setMarketing(me?.marketingConsent || false);
    setNotificationsEnabled(me?.notificationConsent !== false);
  }, [me]);
  const enablePush = async () => {
    ...
    try {
      ...
      setPushStatus("Đã bật thông báo trên thiết bị này.");
    } catch (error: any) {
      setPushStatus(error?.message || "Không bật được thông báo.");
    } finally {
      setPushBusy(false);
    }
  };
  if (!me)
    return (
      <EmptyState
        ...
        text="Dùng OTP điện thoại để xem thiết bị, phiếu sửa và nhận thông báo chính xác."
        action={
          <AppButton primary onClick={onLogin}>
            Đăng nhập bằng OTP
          </AppButton>
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-3xl border border-zinc-200 bg-white p-5">
        <div className="flex items-center gap-3">
          <div className="flex h-14 w-14 items-center justify-center rounded-3xl bg-orange-100 text-xl font-black text-orange-700">
            {(me.displayName || "K").slice(0, 1).toUpperCase()}
          </div>
          <div>
            <p className="font-black text-zinc-900">{me.displayName}</p>
            <p className="mt-1 font-mono text-xs text-zinc-500">
              {me.phoneMasked}
            </p>
          </div>
        ...
        <label className="mt-5 block text-sm font-bold text-zinc-700">
          ...
          <input
            ...
            className="mt-1.5 h-12 w-full rounded-2xl border border-zinc-200 px-3 outline-none focus:border-orange-500"
          />
        </label>
        <label className="mt-4 flex items-start gap-3 rounded-2xl bg-zinc-50 p-3 text-sm">
          <input
            type="checkbox"
          ...
          />
          <span>
            <b>Nhận thông báo nghiệp vụ</b>
            <span className="mt-1 block text-xs leading-5 text-zinc-500">
              Tiến độ sửa chữa và báo giá luôn có trong app; bật push để nhận
              ngay trên điện thoại.
            </span>
          </span>
        </label>
        <label className="mt-3 flex items-start gap-3 rounded-2xl bg-zinc-50 p-3 text-sm">
          <input
            type="checkbox"
            checked={marketing}
          ...
          />
          <span>
            <b>Nhận ưu đãi cá nhân hóa</b>
            <span className="mt-1 block text-xs leading-5 text-zinc-500">
              Chỉ nhận thông tin phù hợp; bạn có thể tắt bất cứ lúc nào.
            </span>
          </span>
        </label>
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <AppButton onClick={() => void enablePush()} disabled={pushBusy}>
            {pushBusy ? "Đang bật…" : "Bật push trên máy này"}
          </AppButton>
          <AppButton onClick={onLogout}>Đăng xuất</AppButton>
        </div>
        {pushStatus && (
          <p className="mt-3 text-xs font-bold text-zinc-600">{pushStatus}</p>
        )}
      </section>
      <section className="rounded-3xl border border-zinc-200 bg-white p-5">
        <h2 className="font-black">Quyền riêng tư & hỗ trợ</h2>
        <p className="mt-2 text-sm leading-6 text-zinc-500">
          PhoneHouse chỉ hiển thị dữ liệu mua hàng, bảo hành và sửa chữa đã liên
          kết với số điện thoại của bạn.
        </p>
      </section>
    </div>
  );
...
function RepairDetail({
  ...
  onQuote: (repair: CustomerRepair) => void;
}) {
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-0 sm:p-6">
      <div className="mx-auto min-h-full max-w-2xl bg-[#fffaf7] sm:min-h-0 sm:rounded-3xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-zinc-200 bg-[#fffaf7]/95 px-4 py-3 backdrop-blur">
          <div>
            <p className="font-black text-zinc-900">{repair.model}</p>
            <p className="font-mono text-[11px] text-zinc-500">
              {repair.code} · {repair.imeiMasked}
            </p>
          </div>
          <button
            aria-label="Đóng"
            onClick={onClose}
            className="flex h-11 w-11 items-center justify-center rounded-2xl hover:bg-zinc-100"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        <div className="space-y-5 p-4 sm:p-6">
          <div className="rounded-3xl bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs text-zinc-500">Trạng thái hiện tại</p>
                <p className="mt-1 text-lg font-black text-zinc-900">
                  {repair.stageLabel}
                </p>
              </div>
              <StatusPill
                label={repair.type === "WARRANTY" ? "Bảo hành" : "Sửa dịch vụ"}
                tone={repair.type === "WARRANTY" ? "green" : "orange"}
              />
            </div>
            {repair.promisedAt && (
              <p className="mt-3 flex items-center gap-2 text-sm text-zinc-600">
                <CalendarDays className="h-4 w-4 text-orange-500" />
                Hẹn trả: <b>{dateTime(repair.promisedAt)}</b>
              </p>
            )}
            {repair.diagnosis && (
              <p className="mt-3 rounded-2xl bg-zinc-50 p-3 text-sm leading-6 text-zinc-700">
                <b>Thông tin kiểm tra:</b> {repair.diagnosis}
              </p>
            )}
          </div>
          {repair.quote.mayDecide && (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm text-orange-800">
                    {money.format(repair.quote.amount)} · phiên bản{" "}
                    {repair.quote.version}
                  </p>
                  <AppButton
                    primary
                    className="mt-3"
                    onClick={() => onQuote(repair)}
                  >
                    Xem & xác nhận báo giá
                  </AppButton>
                </div>
              </div>
            </div>
          ...
          {repair.timeline.length > 0 && (
            <section>
              <h2 className="mb-3 font-black text-zinc-900">Tiến độ xử lý</h2>
                ...
                {repair.timeline.map((item, index) => (
                  ...
                  >
                    <div className="relative flex w-5 shrink-0 justify-center">
                      <span
                        className={classNames(
                          "z-10 mt-1.5 h-3 w-3 rounded-full ring-4 ring-[#fffaf7]",
          ...
          <section className="rounded-3xl border border-zinc-200 bg-white p-4">
            <h2 className="font-black text-zinc-900">Chi phí & thanh toán</h2>
            <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
              <div className="rounded-2xl bg-zinc-50 p-3">
                <p className="text-zinc-500">Tổng</p>
                <p className="mt-1 font-black text-zinc-900">
                  {money.format(repair.payment.finalAmount)}
                </p>
              </div>
              <div className="rounded-2xl bg-emerald-50 p-3">
                <p className="text-emerald-700">Đã thu</p>
                <p className="mt-1 font-black text-emerald-800">
                  {money.format(repair.payment.paidAmount)}
                </p>
              </div>
              <div className="rounded-2xl bg-amber-50 p-3">
                <p className="text-amber-700">Còn lại</p>
                <p className="mt-1 font-black text-amber-800">
                  {money.format(repair.payment.balanceDue)}
                </p>
              </div>
            </div>
          </section>
          <AppButton onClick={() => void onReload()} className="w-full">
...
function QuoteModal({
  ...
  const [otpConfirmation, setOtpConfirmation] = useState<Awaited<
    ReturnType<typeof requestCustomerPhoneOtp>
  > | null>(null);
  const [code, setCode] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  const accept = async () => {
    ...
    try {
      ...
      const confirmation = await requestCustomerPhoneOtp(
        customerAuth.currentUser?.phoneNumber || "",
        "customer-quote-recaptcha",
      );
      setChallenge(result.data);
      setOtpConfirmation(confirmation);
      setStep("otp");
    } catch (e: any) {
      setError(e?.message || "Không thể tạo phiên xác nhận.");
    } finally {
      setLoading(false);
    }
  };
  const confirm = async () => {
    ...
    setError("");
    try {
      if (!otpConfirmation || !challenge)
        throw new Error("Vui lòng yêu cầu lại mã OTP.");
      await otpConfirmation.confirm(code);
      const token = await customerAuth.currentUser?.getIdToken(true);
      if (!token) throw new Error("Vui lòng đăng nhập lại để xác nhận.");
      await decideCustomerQuote(repair.id, {
        decision: "ACCEPT",
      ...
      setError(e?.message || "Không thể xác nhận báo giá.");
    } finally {
      setLoading(false);
    }
  };
  const decision = async (value: "CONSULT" | "REJECT") => {
    ...
    try {
      await decideCustomerQuote(repair.id, {
        ...
        reason:
          value === "CONSULT"
            ? "Khách muốn được tư vấn lại"
            : "Khách từ chối báo giá",
      });
      await onDone();
      onClose();
    } catch (e: any) {
      setError(e?.message || "Không thể ghi nhận lựa chọn.");
    } finally {
      setLoading(false);
    }
  };
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-5">
      <div className="w-full max-w-lg rounded-t-3xl bg-[#fffaf7] p-5 sm:rounded-3xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-black">Xác nhận báo giá</h2>
          <button
            onClick={onClose}
        ...
        <div className="mt-4 rounded-3xl bg-white p-4">
          <p className="text-sm text-zinc-500">
            {repair.model} · phiên bản {repair.quote.version}
          </p>
          <p className="mt-2 text-3xl font-black text-zinc-950">
            {money.format(repair.quote.amount)}
          </p>
          <p className="mt-2 text-xs leading-5 text-zinc-500">
            Giá do PhoneHouse phê duyệt. Khi đồng ý, bạn xác nhận bằng OTP mới
            gửi tới số điện thoại này.
          </p>
        </div>
        {step === "view" ? (
          <div className="mt-4 grid gap-2">
            <p className="text-xs font-bold text-zinc-500">
              Xác nhận chống spam trước khi nhận mã:
            </p>
            <div
              id="customer-quote-recaptcha"
              className="min-h-[78px] overflow-hidden rounded-xl"
            />
            <AppButton
              primary
              className="w-full"
            ...
            >
              {loading ? (
                ...
                "Đồng ý báo giá & nhận OTP"
              )}
            </AppButton>
            <AppButton
              className="w-full"
              onClick={() => void decision("CONSULT")}
              disabled={loading}
            >
              Tôi muốn được tư vấn lại
            </AppButton>
            <button
              onClick={() => void decision("REJECT")}
              disabled={loading}
              className="py-3 text-xs font-bold text-zinc-500 underline"
            >
              Từ chối sửa chữa
            </button>
          </div>
          ...
          <div className="mt-4">
            <p className="text-sm font-bold text-zinc-700">Nhập mã OTP mới</p>
            <p className="mt-1 text-xs text-zinc-500">
              Phiên xác nhận hết hạn lúc {dateTime(challenge?.expiresAt)}
            </p>
            <input
              autoFocus
              inputMode="numeric"
              ...
              className="mt-3 h-14 w-full rounded-2xl border border-zinc-200 text-center text-2xl font-black tracking-[.4em] outline-none focus:border-orange-500"
              placeholder="••••••"
            />
            <AppButton
              primary
...
function CustomerRepairIntakeView({
  bootstrap,
  devices,
  onClose,
  onDone,
  ...
  const initialBranchId = bootstrap.branches.some(
    (branch) => branch.id === initialDevice?.branchId,
  )
  ...
  const [requestType, setRequestType] = useState<"REPAIR" | "WARRANTY">(
    "REPAIR",
  );
  ...
  const [operationKey] = useState(
    () => `csr-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  ...
  const chooseLinkedDevice = (device: CustomerDevice) => {
    ...
    setImei("");
    setModel("");
    if (bootstrap.branches.some((branch) => branch.id === device.branchId))
  ...
  const stepValid =
    step === 1
      ? manualDevice
        ? /^\d{15}$/.test(imei) && model.trim().length >= 2
  ...
  const submit = async () => {
    ...
    try {
      ...
      for (const file of files.slice(0, 6))
        await uploadCustomerEvidence(result.data.id, file);
      await onDone();
      onClose();
    } catch (cause: any) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Phiếu tiếp nhận sửa chữa"
      className="fixed inset-0 z-[80] overflow-y-auto bg-[#fffaf7] sm:bg-black/50 sm:p-6"
    >
      <div className="mx-auto flex min-h-full w-full max-w-2xl flex-col bg-[#fffaf7] sm:min-h-0 sm:max-h-[calc(100vh-3rem)] sm:rounded-[2rem] sm:shadow-2xl">
        <header className="sticky top-0 z-20 border-b border-zinc-200 bg-[#fffaf7]/95 px-4 py-3 backdrop-blur sm:rounded-t-[2rem]">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-base font-black">Phiếu tiếp nhận online</p>
            ...
            <button
              type="button"
              aria-label="Đóng phiếu tiếp nhận"
              onClick={onClose}
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl hover:bg-zinc-100"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          <div
            ...
            aria-label={`Tiến độ ${step} trên 4 bước`}
          >
            {[1, 2, 3, 4].map((value) => (
              <span
                key={value}
                className={classNames(
                  "h-1.5 rounded-full",
                  value <= step ? "bg-[#ff4b16]" : "bg-zinc-200",
                )}
              />
            ))}
          </div>
        ...
        <main className="flex-1 overflow-y-auto px-4 py-5 sm:px-6">
          {step === 1 && (
            <section>
              ...
              <p className="mt-1 text-sm leading-6 text-zinc-500">
                Máy đã liên kết được lấy trực tiếp từ hồ sơ PhoneHouse, bạn
                không phải nhập lại model hoặc IMEI.
              </p>
              {devices.length > 0 && (
                <div className="mt-4 space-y-2">
                  {devices.map((device) => (
                    <button
                      type="button"
                      key={device.id}
                      onClick={() => chooseLinkedDevice(device)}
                      className={classNames(
                        "flex min-h-[68px] w-full items-center justify-between gap-3 rounded-2xl border p-3 text-left transition",
                        !manualDevice && deviceId === device.id
                          ? "border-orange-500 bg-orange-50 ring-2 ring-orange-100"
                          : "border-zinc-200 bg-white hover:border-orange-200",
                      )}
                    >
                      <span className="min-w-0">
                        <b className="block truncate text-sm text-zinc-900">
                          {device.model || "Thiết bị PhoneHouse"}
                        </b>
                        <small className="mt-1 block font-mono text-xs text-zinc-500">
                          {device.imeiMasked} ·{" "}
                          {device.branchName || "PhoneHouse"}
                        </small>
                      </span>
              ...
              <button
                type="button"
                onClick={chooseManualDevice}
                className={classNames(
                  "mt-3 flex min-h-12 w-full items-center justify-between rounded-2xl border px-4 text-left text-sm font-black",
                  manualDevice
                    ? "border-orange-500 bg-orange-50 text-orange-800"
                    : "border-zinc-200 bg-white text-zinc-700",
                )}
              >
                <span>Máy khác chưa liên kết</span>
                {manualDevice && <Check className="h-5 w-5" />}
              </button>
              {manualDevice && (
                <div className="mt-4 rounded-3xl border border-zinc-200 bg-white p-4">
                  <p className="text-xs leading-5 text-zinc-500">
                    Nhập thông tin trên máy. Nhân viên sẽ xác minh IMEI khi tiếp
                    nhận thực tế.
                  </p>
          ...
          {step === 2 && (
            <section>
              ...
              <p className="mt-1 text-sm leading-6 text-zinc-500">
                Chọn một nhóm lỗi chính; mô tả thêm dấu hiệu để kỹ thuật chuẩn
                bị chính xác hơn.
              </p>
              <div className="mt-4 grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  <p className="mt-2 text-sm font-black">Sửa dịch vụ</p>
                ...
                >
                  ...
                  <p className="mt-2 text-sm font-black">Bảo hành</p>
                  <p className="mt-1 text-[11px] leading-4 text-zinc-500">
                    Xác minh điều kiện
                  </p>
                </button>
              </div>
              <p className="mt-5 text-sm font-black text-zinc-800">
                Nhóm lỗi chính
              </p>
                ...
                {CUSTOMER_REPAIR_ISSUES.map((issue) => (
                  ...
                  >
                    <span className="flex items-start justify-between gap-2">
                      <b className="text-sm text-zinc-900">{issue.label}</b>
                      {issueCode === issue.code && (
              ...
              <label className="mt-5 block text-sm font-bold">
                ...
                <textarea
                  value={description}
                  onChange={(event) =>
                    setDescription(event.target.value.slice(0, 3000))
                  }
                  className="mt-1.5 min-h-32 w-full rounded-2xl border border-zinc-200 p-3 outline-none focus:border-orange-500"
                  placeholder="Ví dụ: máy bắt đầu không nhận sạc từ tối qua, đã thử đổi cáp nhưng vẫn chập chờn…"
                />
              </label>
          ...
          {step === 3 && (
            <section>
              ...
              <p className="mt-1 text-sm leading-6 text-zinc-500">
                Không bắt buộc, nhưng ảnh rõ lỗi và toàn bộ máy giúp tiếp nhận
                nhanh hơn.
              </p>
              <label className="mt-5 flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-3xl border border-dashed border-orange-300 bg-orange-50 p-4 text-center">
                ...
                />
                <CameraIcon />
                <b className="mt-3 text-sm text-orange-900">
                  Chụp ảnh hoặc chọn từ điện thoại
                </b>
                <span className="mt-1 text-xs text-orange-700">
                  Tối đa 6 tệp ảnh/video
                </span>
              </label>
              {files.length > 0 && (
                <div className="mt-3 rounded-2xl bg-white p-3 text-sm">
                  <b>Đã chọn {files.length} tệp</b>
              ...
              <div className="mt-4 rounded-2xl bg-sky-50 p-3 text-xs leading-5 text-sky-800">
                <b>Gợi ý:</b> Chụp màn hình đang lỗi, mặt trước/sau và vị trí
                trầy vỡ. Không cần quay lại IMEI nếu máy đã liên kết.
              </div>
            </section>
          )}
            ...
            <section>
              ...
              <label className="mt-4 block text-sm font-bold">
                ...
                <select
                  ...
                  className="mt-1.5 h-12 w-full rounded-2xl border border-zinc-200 bg-white px-3 outline-none focus:border-orange-500"
                >
              ...
              <label className="mt-3 block text-sm font-bold">
                ...
                <input
                  ...
                  onChange={(event) => setPreferredVisitAt(event.target.value)}
                  className="mt-1.5 h-12 w-full rounded-2xl border border-zinc-200 bg-white px-3 outline-none focus:border-orange-500"
                />
              </label>
              <div className="mt-5 space-y-3 rounded-3xl border border-zinc-200 bg-white p-4 text-sm">
                <div>
                  <p className="text-xs text-zinc-500">Thiết bị</p>
                  <p className="mt-1 font-black">
                    {manualDevice ? model : selectedDevice?.model}
                  </p>
                  <p className="mt-0.5 font-mono text-xs text-zinc-500">
                    {manualDevice ? imei : selectedDevice?.imeiMasked}
                  </p>
                </div>
                <div className="border-t border-zinc-100 pt-3">
                  <p className="text-xs text-zinc-500">Yêu cầu</p>
                  <p className="mt-1 font-bold">
                    {requestType === "WARRANTY" ? "Bảo hành" : "Sửa dịch vụ"} ·{" "}
                    {selectedIssue?.label}
                  </p>
                  <p className="mt-1 leading-5 text-zinc-600">{description}</p>
                </div>
                <div className="border-t border-zinc-100 pt-3">
                  <p className="text-xs text-zinc-500">Nơi tiếp nhận</p>
                  <p className="mt-1 font-bold">
                    {bootstrap.branches.find((branch) => branch.id === branchId)
                      ?.name || "Chưa chọn"}
                  </p>
                  <p className="mt-1 text-xs text-zinc-500">
                    {files.length
                      ? `${files.length} tệp bằng chứng`
                      : "Không có tệp đính kèm"}
                  </p>
                </div>
              </div>
              <p className="mt-3 text-xs leading-5 text-zinc-500">
                Đây là yêu cầu tiếp nhận online. Nhân viên sẽ xác minh tình
                trạng máy, IMEI và điều kiện bảo hành trước khi tạo phiếu kỹ
                thuật chính thức.
              </p>
            </section>
          )}
          {error && (
            <p
              role="alert"
              className="mt-4 rounded-2xl bg-rose-50 p-3 text-xs font-bold leading-5 text-rose-700"
            >
              {error}
            </p>
          )}
...
function CameraIcon() {
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-orange-600">
...
function DeviceDetail({
  ...
  device: CustomerDevice & { repairHistory?: CustomerRepair[] };
  onClose: () => void;
  onRepair: () => void;
}) {
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-0 sm:p-6">
      <div className="mx-auto min-h-full max-w-xl bg-[#fffaf7] sm:min-h-0 sm:rounded-3xl">
        <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3">
          <div>
            <p className="font-black">{device.model}</p>
        ...
        <div className="space-y-4 p-4">
          <section className="rounded-3xl border border-zinc-200 bg-white p-4">
            <div className="flex items-center justify-between">
              <p className="font-black">Bảo hành</p>
              <StatusPill
                label={
                  device.warrantyStatus === "ACTIVE"
                    ? "Còn hạn"
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
              <div>
                <p className="text-xs text-zinc-500">Ngày mua</p>
                <p className="mt-1 font-bold">{dateOnly(device.purchaseAt)}</p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Hết hạn</p>
                <p className="mt-1 font-bold">
                  {dateOnly(device.warrantyUntil)}
                </p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Chi nhánh</p>
                <p className="mt-1 font-bold">
                  {device.branchName || "PhoneHouse"}
                </p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Hóa đơn</p>
          ...
          <AppButton primary className="w-full" onClick={onRepair}>
            <Wrench className="mr-2 inline h-4 w-4" />
            Yêu cầu sửa/bảo hành
          </AppButton>
          {device.repairHistory?.length ? (
            <section>
              <h2 className="mb-2 font-black">Lịch sử xử lý</h2>
              <div className="space-y-2">
                {device.repairHistory.map((item) => (
                  <div
                    key={item.id}
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <StatusPill label={item.stageLabel} tone="blue" />
                    </div>
                    <p className="mt-1 text-xs text-zinc-500">
                      {item.type === "WARRANTY" ? "Bảo hành" : "Sửa dịch vụ"} ·{" "}
                      {dateOnly(item.receivedAt)}
                    </p>
                  </div>
                ))}
              </div>
...
function PromotionDetail({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 sm:items-center sm:p-5">
      <div className="w-full max-w-lg rounded-t-3xl bg-[#fffaf7] p-5 sm:rounded-3xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-black">{promotion.title}</h2>
          <button
            onClick={onClose}
            className="flex h-11 w-11 items-center justify-center rounded-2xl"
          >
        ...
        {promotion.conditions.length > 0 && (
          <div className="mt-4 rounded-2xl bg-white p-4">
            ...
            <ul className="mt-2 list-disc space-y-1 pl-5 text-sm leading-5 text-zinc-600">
        ...
        <p className="mt-4 text-xs font-bold text-zinc-500">
          Hiệu lực đến {dateTime(promotion.endsAt)} ·{" "}
          {promotion.allBranches ? "Toàn hệ thống" : "Theo chi nhánh áp dụng"}
        </p>
...
function NotificationsPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 sm:items-center sm:p-5">
      <div className="max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-t-3xl bg-[#fffaf7] p-5 sm:rounded-3xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-black">Thông báo</h2>
          <button
            onClick={onClose}
            className="flex h-11 w-11 items-center justify-center rounded-2xl"
          >
...
function ChatPanel({
  ...
  const quick = [
    "Máy của tôi sửa tới đâu?",
    "Máy còn bảo hành không?",
    "Có ưu đãi nào phù hợp?",
    "Tôi muốn gặp CSKH",
  ];
  useEffect(() => {
    if (me)
      void createCustomerConversation()
        .then((result) => setConversationId(result.data.id))
        .catch((e) => setError(e?.message || "Không mở được chat."));
  }, [me]);
  const send = async (value = input) => {
    ...
    try {
      if (!me) {
        const result = await customerPublicChat(value);
        setPublicReply(result.data.reply);
      } else {
        ...
        const result = await sendCustomerConversationMessage(id, value);
  ...
  const handoff = async () => {
    ...
    setLoading(true);
    try {
      await handoffCustomerConversation(conversationId);
      setMessages((current) => [
        ...current,
        {
          id: `handoff-${Date.now()}`,
          sender: "BOT",
          senderName: "PhoneHouse Care",
          content:
            "Đã chuyển cuộc trò chuyện cho CSKH. Nhân viên sẽ phản hồi trong app.",
          timestamp: new Date().toISOString(),
        },
      ]);
    } catch (e: any) {
      setError(e?.message || "Không chuyển được cho CSKH.");
    } finally {
      setLoading(false);
    }
  };
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/40 sm:items-center sm:p-5">
      <div className="flex h-[min(760px,92vh)] w-full max-w-lg flex-col rounded-t-3xl bg-[#fffaf7] sm:rounded-3xl">
        <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-black">PhoneHouse Care</p>
              <p className="text-xs text-emerald-600">
                {me
                  ? "Đã đăng nhập · hỗ trợ cá nhân"
                  : "Hỗ trợ thông tin công khai"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="rounded-3xl bg-white p-4 text-sm leading-6 text-zinc-700 shadow-sm">
            Xin chào! Tôi có thể hỗ trợ bảo hành, tiến độ sửa chữa, báo giá, ưu
            đãi và chi nhánh.{" "}
            {me ? "" : "Đăng nhập OTP để hỏi thông tin riêng của bạn."}
          </div>
          ...
          {publicReply && (
            <div className="mt-3 rounded-3xl bg-sky-50 p-4 text-sm leading-6 text-sky-900">
              {publicReply}
            </div>
          )}
          {messages.map((message) => (
            <div
              key={message.id}
              className={classNames(
                "mt-3 max-w-[88%] rounded-3xl p-3 text-sm leading-6",
                message.sender === "CUSTOMER"
                  ? "ml-auto bg-[#ff4b16] text-white"
        ...
        <div className="border-t border-zinc-200 p-3">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={1}
              className="min-h-11 flex-1 resize-none rounded-2xl border border-zinc-200 bg-white p-3 text-sm outline-none focus:border-sky-500"
          ...
          {me && (
            <button
              ...
              disabled={loading}
              className="mt-2 text-xs font-black text-sky-700 underline"
            >
              Gặp nhân viên CSKH
            </button>
          )}
...
export default function CustomerPortalApp() {
  ...
  const [quotePage, setQuotePage] = useState(() =>
    window.location.pathname.toLowerCase().startsWith("/khach-hang/bao-gia"),
  );
  const [overlay, setOverlay] = useState<Overlay>(null);
  const [bootstrap, setBootstrap] = useState<CustomerBootstrap>({
    brand: {
      name: "PhoneHouse",
      slogan: "An tâm mua sắm · Tận tâm hậu mãi",
      hotline: "",
      supportEmail: "",
    },
    branches: [],
  ...
  const [selectedRepair, setSelectedRepair] = useState<CustomerRepair | null>(
    null,
  );
  const [selectedDevice, setSelectedDevice] = useState<
    (CustomerDevice & { repairHistory?: CustomerRepair[] }) | null
  >(null);
  ...
  useEffect(() => {
    ...
    document
      ...
      ?.setAttribute(
        "content",
        "Theo dõi bảo hành, sửa chữa, báo giá và ưu đãi dành cho khách hàng PhoneHouse.",
      );
    document
      .querySelector<HTMLMetaElement>('meta[name="application-name"]')
      ?.setAttribute("content", "PhoneHouse Care");
    document
      .querySelector<HTMLMetaElement>('meta[name="apple-mobile-web-app-title"]')
      ?.setAttribute("content", "PhoneHouse Care");
    if ("serviceWorker" in navigator)
      void navigator.serviceWorker
        .register("/customer-sw.js", { scope: "/khach-hang" })
        .catch((error) => console.warn("[PhoneHouse Care SW]", error));
  }, []);

  const refresh = useCallback(
    async (authenticated = Boolean(customerAuth.currentUser?.phoneNumber)) => {
      setLoading(true);
      setError("");
      try {
        const publicData = await customerPublicBootstrap();
        setBootstrap(publicData.data);
        if (authenticated && customerAuth.currentUser?.phoneNumber) {
          const account = await customerMe();
          setMe(account.data);
          const [deviceData, repairData, promotionData, notificationData] =
            await Promise.all([
              customerDevices(),
              customerRepairs(),
              customerPromotions(),
          ...
          setRepairs(repairData.data.items);
          setRequests(repairData.data.requests);
          ...
          setRepairs([]);
          setRequests([]);
          setPromotions(publicData.data.promotions);
          setNotifications([]);
        }
      } catch (e: any) {
        setError(e?.message || "Không tải được dữ liệu PhoneHouse Care.");
      } finally {
        setLoading(false);
      }
    },
  ...
  useEffect(() => {
    void refresh(Boolean(customerAuth.currentUser?.phoneNumber));
    const unsub = onAuthStateChanged(customerAuth, (user) => {
      void refresh(Boolean(user?.phoneNumber));
    });
    return () => unsub();
  }, [refresh]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    void customerRepair(repairId)
      ...
      .catch((error) =>
        setError(
          error?.message || "Không mở được phiếu sửa chữa từ thông báo.",
        ),
      );
  }, [me]);
  useEffect(() => {
  ...
  const go = (tab: PortalTab) => {
    setQuotePage(false);
    setActiveTab(tab);
    window.history.pushState(null, "", portalPaths[tab]);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };
  const openQuotePage = () => {
    setQuotePage(true);
    window.history.pushState(null, "", "/khach-hang/bao-gia");
    window.scrollTo({ top: 0, behavior: "smooth" });
  };
  const saveMe = async (input: Partial<CustomerMe>) => {
    const result = await updateCustomerMe(input);
    setMe(result.data);
  };
  const enablePush = async () => {
  ...
  const openRepair = (repair: CustomerRepair) => {
    setSelectedRepair(repair);
    setOverlay("repair");
  };
  const reloadRepair = async () => {
    if (!selectedRepair) return;
    const result = await customerRepair(selectedRepair.id);
    setSelectedRepair(result.data);
    await refresh(true);
  };
  const content = useMemo(() => {
    if (loading && !bootstrap.generatedAt)
      return (
      ...
      return (
        ...
        >
          <QuickQuoteMiniweb
            ...
            hotline={bootstrap.brand.hotline}
            promotions={promotions}
          />
        </React.Suspense>
      );
    ...
    if (activeTab === "repairs")
      return (
        <RepairsPage
          ...
          requests={requests}
          onOpen={openRepair}
          onRequest={() => (me ? setOverlay("request") : setOverlay("login"))}
          onLogin={() => setOverlay("login")}
        />
      );
    if (activeTab === "promotions")
      return (
        <PromotionsPage promotions={promotions} onOpen={setSelectedPromotion} />
      );
    ...
    return (
      <HomePage
        ...
        promotions={promotions}
        onTab={go}
        onOpenRepair={() => (me ? setOverlay("request") : setOverlay("login"))}
        onOpenQuote={openQuotePage}
  ...
  return (
    <div className="min-h-screen bg-[#fffaf7] text-zinc-900">
      <Header
        ...
        onLogin={() => setOverlay("login")}
      />
      <main className="mx-auto max-w-6xl px-4 pb-28 pt-5 sm:px-6 lg:pb-8">
        {error && (
          <div
            role="alert"
            className="mb-4 flex items-start gap-2 rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold leading-5 text-rose-700"
          >
      ...
      <button
        ...
        className="fixed bottom-24 right-4 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-sky-600 text-white shadow-xl shadow-sky-200 transition hover:bg-sky-700 sm:bottom-6 sm:right-6"
      >
      ...
      {overlay === "quote" && selectedRepair && (
        <QuoteModal
          repair={selectedRepair}
          onClose={() => setOverlay("repair")}
          onDone={async () => {
            await reloadRepair();
          }}
        />
      )}
      {overlay === "device" && selectedDevice && (
        <DeviceDetail
      ...
      {overlay === "promotion" && selectedPromotion && (
        <PromotionDetail
      ...
      {overlay === "notifications" && (
        <NotificationsPanel
          ...
          onClose={() => setOverlay(null)}
          onRead={async (id) => {
            await readCustomerNotification(id);
            ...
            if (target?.url?.startsWith("/khach-hang"))
